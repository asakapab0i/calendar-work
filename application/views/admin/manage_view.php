...
$(document).ready(function() {
  $(function() {
    $( ".edit_user_dialog,.edit_dialog , #add-dialog " ).dialog({
       autoOpen: false,
        minWidth: 600,
        minHeight: 300,
        resizable: false,
 ...
 $( ".admin_edit_user" ).click(function () {
    var currentId = $(this).attr('data-id');
      //alert(currentId);
       ...
       $.ajax({
       type: "POST",
        url: "<?php echo base_url().'admin/edit_user/"+currentId+"/'?>",
    
      data: {myid: currentId}
        }).done(function(data) {
        $('.edit_user_dialog').html(data);
        });
...
$(document).ready(function() {
  $(function() {
    $( ".pic_dialog, .change_dialog, #user-dialog" ).dialog({
       autoOpen: false,
        minWidth: 390,
        minHeight: 100,
        resizable: false,
    ...
    <div class="span12">
     ...
     <?php endforeach;?>        
          ...
          <div class="navbar-inner">
                      <a class="brand" href="#">USERS LITS</a> <a href="#" id="add-user" data-id="event" class="btn btn-default"><span class="icon-large icon-plus-sign"></span>Add User</a>
          </div>
    ...
    <tbody>
                  ...
                  <?php foreach ($all_users as $row):?>
                    <tr>
                       ...
                       <td><a href="#" onClick="return false;"><div class="admin_edit_user" data-id="<?=$row->id?>"><img src="<?php echo base_url();?>assets/images/edit.png"/></div> </a></td>
                       

                        <td><a href="<?php echo base_url() . "admin/delete_user/{$row->id}"?>"onClick='return confirm("Are you sure you want to delete, <?=$row->email?>?")'><img src="<?php echo base_url();?>assets/images/delete.png"/></a></td>
                        <td>  <img style="width:50px; height:50px;"src="<?php echo base_url();?>assets/images/user_pic/<?=$row->image?>"/></td>
                       <td><?=$row->email?></td>
                       <td><?=$row->password?></td>
                       <td><?=$row->type?></td>
...
<div class="edit_user_dialog" title="Edit User"></div><!-- this one is for edit category -->
<!-- end here -->
...
<!-- user dialog -->
<div id="user-dialog" title="Add User">
